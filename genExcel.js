...
async function writeExcel() {
  await GoodRxIdModel.find({}, function(err, data) {
    if (err) console.log(err);
    else {
      let wb = createDoc(data);
      wb.write('drug_forms.xlsx', function(err, stats) {
          if (err) console.log(err);
          console.log("done.");
          process.exit();
      });
